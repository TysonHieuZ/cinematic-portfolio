...
// YouTube video ID for "Daylight - David Kushner"
const VIDEO_ID = 'MoN9ql6Yymw'
...
function onPlayerStateChange(event) {
  ...
  if (event.data === 1) {
    ...
    if (event.data === 0) { progress.value = 0; currentTime.value = 0 }
  }
}
...
function seek(e) {
  ...
  player.seekTo(pct * duration.value, true)
}
...
<template>
  <div class="music-player" :class="{ 'music-player--expanded': isExpanded }">
    ...
    <div v-if="!isExpanded" class="music-player__pill">
      <button
        class="music-player__pill-play"
        :aria-label="isPlaying ? 'Pause music' : 'Play Daylight – David Kushner'"
        type="button"
        @click="togglePlay"
      >
        <!-- Bars when playing -->
        ...
        <!-- Play icon -->
        <svg v-else viewBox="0 0 24 24" fill="currentColor" class="music-player__icon" aria-hidden="true">
          <polygon points="6,4 20,12 6,20"/>
        </svg>
      </button>

      <div class="music-player__pill-info" @click="isExpanded = true">
        <span class="music-player__pill-title">Daylight</span>
        <span class="music-player__pill-artist">David Kushner</span>
      </div>

      <button
        class="music-player__pill-expand"
    ...
    <div v-else class="music-player__card">
      ...
      <div class="music-player__track">
        <p class="music-player__track-title">Daylight</p>
        <p class="music-player__track-artist">David Kushner</p>
      </div>
      ...
      <div class="music-player__controls">
        ...
        <a
          class="music-player__ctrl-btn"
          href="https://www.youtube.com/watch?v=MoN9ql6Yymw"
          target="_blank"
...
.music-player__icon { width: 10px; height: 10px; }
...
.music-player__pill-expand svg { width: 14px; height: 14px; }
...
.music-player__art-inner {
  width: 100px; height: 100px;
}
.music-player__art-inner--spin {
...
.music-player__ctrl-btn svg { width: 16px; height: 16px; }
...
.music-player__play-btn svg { width: 18px; height: 18px; }
